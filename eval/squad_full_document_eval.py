...
class RankedParagraphQuestion(ContextAndQuestion):
    ...
    def __init__(self, question: List[str], answer: Optional[Answer],
                 question_id: str, paragraph: Paragraph, rank: int):
...
class RecordParagraphSpanPrediction(Evaluator):
    ...
    def evaluate(self, data: List[RankedParagraphQuestion], true_len, **kargs):
        ...
        results["para_number"] = np.array([x.paragraph.paragraph_num for x in data])
...
def main():
    ...
    print("Breaking up into questions")

    questions = []
    if args.ranked_questions == "none":
        if args.n_paragraphs is not None:
            raise ValueError()
        for doc in docs:
            all_paragraphs = doc.paragraphs
            for ix, para in enumerate(doc.paragraphs):
                for q in para.questions:
                    for other_ix, other_para in enumerate(all_paragraphs):
                        ans = q.answer if other_ix == ix else None
                        questions.append(RankedParagraphQuestion(
                            q.words, ans, q.question_id, other_para, other_ix))
    else:
        ...
        for q in ranker.ranked_questions(docs):
            for i, p in enumerate(q.paragraphs):
                questions.append(RankedParagraphQuestion(q.paragraphs,
