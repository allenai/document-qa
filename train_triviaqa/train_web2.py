...
from nn.layers import NullBiMapper, NullMapper, SequenceMapperSeq, ReduceLayer, Conv1d, HighwayLayer, FullyConnected, \
    ChainBiMapper, DropoutLayer, ConcatWithProduct, ResidualLayer, WithProjectedProduct, MapperSeq, LeakyRelu, \
    TruncatedNormalInit
from nn.recurrent_layers import BiRecurrentMapper, LstmCellSpec, BiDirectionalFusedLstm, FusedRecurrentEncoder, \
    EncodeOverTime, CudnnGru, CudnnLstm
...
def main():
    ...
    model = Attention(
        ...
        match_encoder=DropoutLayer(0.8),
        predictor= BoundsPredictor(
            ChainBiMapper(
                first_layer=CudnnLstm(100),
                second_layer=CudnnLstm(100)
            )
        )
    )
