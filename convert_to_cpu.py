...
from data_processing.qa_training_data import ParagraphAndQuestionSpec, ParagraphAndQuestion
from doc_qa_models import Attention
from model_dir import ModelDir
from nn.recurrent_layers import BiRecurrentMapper, CompatGruCellSpec
...
manually, which means this will only for our models.
"""
...
def convert(model_dir, output_dir):
    ...
    model = md.get_model()
    if not isinstance(model, Attention):
    ...
    other.layers[1] = BiRecurrentMapper(CompatGruCellSpec(140))

    pred = model.predictor.predictor
    pred.first_layer = BiRecurrentMapper(CompatGruCellSpec(140))
    pred.second_layer = BiRecurrentMapper(CompatGruCellSpec(140))

    with open(join(output_dir, "model.npy"), "wb") as f:
        pickle.dump(model, f)

    print("Testing...")
    with open(join(output_dir, "model.npy"), "rb") as f:
        model = pickle.load(f)
